    		...
    		for nn in "${num_neighbors[@]}"; do
		        for full_subgraph in "${full_subgraphs[@]}"; do
		            for eval_method in "${eval_methods[@]}"; do
		                ...
		                python -u main.py --data_path ${data_path} --gnn_model ${gnn_model} --graph_type ${graph_type} --num_neighbors ${nn} --dim ${d} --full_subgraph ${full_subgraph} --eval_method ${eval_method}
